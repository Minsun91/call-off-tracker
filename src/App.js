...
const getOrdersCollectionPath = () =>
  `artifacts/${APP_ID}/public/data/warehouse_orders`;
...
const fetchUserRole = async (userId) => {
  ...
  if (userSnap.exists()) {
    ...
    // 기본 SCM으로 등록
    await setDoc(userRef, { role: USER_ROLES.SCM });
    return USER_ROLES.SCM;
  }
};
...
// ✅ 신규 오더 생성 폼
// ----------------------------------------------------
function CreateOrderForm({ userId, userRole }) {
  ...
  if (userRole !== USER_ROLES.SCM) return null; // Warehouse는 생성 못함
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <input type="text" placeholder="Customer Name" value={customer} onChange={e => setCustomer(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" />
          <input type="date" placeholder="Delivery Date" value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" />
          <input type="number" placeholder="Pallet Quantity" value={palletQuantity} onChange={e => setPalletQuantity(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" />
          <input type="text" placeholder="Delivery Location (or 'Pickup')" value={deliveryLocation} onChange={e => setDeliveryLocation(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg" />
        </div>
        <div className="flex items-center mb-4">
          <input id="docSentCheckbox" type="checkbox" checked={isDocSent} onChange={e => setIsDocSent(e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded" />
          <label htmlFor="docSentCheckbox" className="ml-2 text-sm text-gray-700">
            Invoice + Packing List sent to warehouse via email
          </label>
        </div>
        <button type="submit" disabled={isSubmitting} className="w-full px-6 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 disabled:bg-gray-400">
...
function OrderCard({ order, userRole }) {
  ...
  const handleStatusChange = async (newStatus) => {
    if (userRole !== USER_ROLES.WAREHOUSE) return; // SCM은 상태 변경 못함
    try {
  ...
  const handleDetailsUpdate = async (e) => {
    e.preventDefault();
    if (userRole !== USER_ROLES.WAREHOUSE) return; // SCM은 창고 메모/POD 수정 못함
    setIsUpdating(true);
    try {
      await updateDoc(orderRef, { warehouseNotes: notes, podUrl });
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (userRole !== USER_ROLES.SCM || order.createdBy !== order.userId) return;
    try {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg flex flex-col">
      ...
      <div className="p-4 flex-grow">
        ...
        <p className="text-sm text-gray-600 mt-1"><strong>Location:</strong> {order.deliveryLocation}</p>
      </div>

      <div className="p-4 bg-gray-50 border-t">
        <form onSubmit={handleDetailsUpdate} className="space-y-3">
          <input type="text" placeholder="Price/ETA notes..." value={notes} onChange={e => setNotes(e.target.value)} className="w-full p-2 border rounded-lg text-sm" />
          <input type="text" placeholder="Please send us POD when you receive it." value={podUrl} onChange={e => setPodUrl(e.target.value)} className="w-full p-2 border rounded-lg text-sm" />
          {userRole === USER_ROLES.WAREHOUSE && (
            <button type="submit" disabled={isUpdating} className="w-full px-4 py-2 bg-blue-500 text-white text-sm rounded-lg hover:bg-blue-600 disabled:bg-gray-300">
      ...
      <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-2 border-t">
        ...
        {userRole === USER_ROLES.SCM && order.createdBy === order.userId && (
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {userId && <CreateOrderForm userId={userId} userRole={userRole} />}
